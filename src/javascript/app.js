Ext.define("sankey-dependency-by-release", {
    ...
    launch: function() {
        if (this.getContext().getTimeboxScope() && this.getContext().getTimeboxScope().type === 'release'){
            this._update(this.getContext().getTimeboxScope());
        } else {
            this.add({
    ...
    loadDataWS: function (release) {
        ...
        wss.load({
            ...
            callback: function (records, options, success) {
                ...
                var recs = _(records)
                    .filter(function (rec) { return rec.data.Predecessors.Count + rec.data.Successors.Count; }, this)
                    .map(function (rec) {
                        var ret = [];
                ...
                this.logger.log('processed records', recs);

                Deft.Promise.all(recs).then(function () {
                    _(records)
                        .filter(function (rec) { return rec.data.Predecessors.Count + rec.data.Successors.Count; })
                        .each(function (rec) {
